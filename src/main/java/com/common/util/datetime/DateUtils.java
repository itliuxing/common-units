...
public class DateUtils {
	...
	public static Date simpleDateFormat( String date ,int style ) throws ParseException{
		if(StringUtils.isBlank(date) || style >= dateFormat.length || style < 0 ){
			return null ;
		}else{
	 ...
	 * @return	yyyy-MM-dd
	 */
	public static Date getCurrentDateBeforeDate( Date data ,int n) {
		Calendar day = Calendar.getInstance() ;
		day.setTime( data );
		day.add(Calendar.DATE, -n);
		return day.getTime();
	 ...
	 * @return
	 */
	public static Date getForeverTime( Date date , int year ){
		Calendar calendar = Calendar.getInstance();
		if( date != null ) {
			calendar.setTime( date );
		}
	 ...
	 * @throws ParseException 日期格式化异常则不做处理
	 */
	public static int timeDistance( Date start,Date stop  ) {
	 ...
	 * @return yyyy-MM-dd
	 */
	public static Date getParamDateBefore(Date date, int n) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
	 ...
	 * @return
	 */
	public static long nextMinuteDistance( int minute ){
		Calendar calendar = Calendar.getInstance() ;
		calendar.setTimeInMillis( System.currentTimeMillis() );
		int unroundedMinutes = calendar.get(Calendar.MINUTE);
		int mod = unroundedMinutes % minute;
		calendar.add(Calendar.MINUTE, mod == 0 ? minute : minute - mod );
		calendar.set(Calendar.SECOND, 0);
		calendar.set(Calendar.MILLISECOND, 0);
		long sleep = ( calendar.getTimeInMillis() - System.currentTimeMillis() ) ;
		logger.info( " 下一个时间点的到来，还有 :{} 毫秒................. " , sleep );
		return sleep ;
	}


	public static void main(String[] args) {
		nextMinuteDistance( 10 ) ;

		Date date = new Date();
		Date getDate = getParamDateBefore(date, 6);

		String dateStr = simpleDateFormat(date, 0);
		String getDateStr = simpleDateFormat(getDate, 0);
		System.out.println(dateStr + "----" + getDateStr);
	}
	...
	public static List<String> getDays(String startTime, String endTime) {
		...
		DateFormat dateFormats = new SimpleDateFormat( dateFormat[3] );
		try {
			Date start = dateFormats.parse(startTime);
			Date end = dateFormats.parse(endTime);
			...
			while (tempStart.before(tempEnd)) {
				days.add(dateFormats.format(tempStart.getTime()));
